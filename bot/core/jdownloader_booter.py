...
class JDownloader(MyJdApi):
    ...
    async def boot(self, _retries=0):
        import os
        if not os.path.exists("/JDownloader"):
            self.is_connected = False
            self.error = "JDownloader directory /JDownloader not found!"
            LOGGER.warning("JDownloader directory /JDownloader not found. JDownloader is disabled.")
            return
