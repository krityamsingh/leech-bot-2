...
async def update_nzb_options():
    import shutil
    if Config.DISABLE_NZB or not Config.USENET_SERVERS or not shutil.which(BinConfig.SABNZBD_NAME):
        return
...
async def start_web_server():
    LOGGER.info("Web server is disabled by request. Only starting the bot.")
    return
...
async def load_configurations():
    ...
    import shutil
    cmd = f'chmod 600 .netrc && chmod +x setpkgs.sh && ./setpkgs.sh {BinConfig.ARIA2_NAME} "{service_cores}" {Config.CPU_LIMIT}'
    if not Config.DISABLE_NZB and shutil.which(BinConfig.SABNZBD_NAME):
    ...
    elif not TorrentManager.qbittorrent:
        LOGGER.warning("qBittorrent not available. Skipping preferences setup.")
    else:
        try:
            await TorrentManager.qbittorrent.app.set_preferences(qbit_options)
        except Exception as e:
