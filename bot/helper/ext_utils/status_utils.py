...
def _download_disk_usage():
    import bot

    download_dir = (
        getattr(bot, "DOWNLOAD_DIR", None)
        or Config.DOWNLOAD_DIR
        or ospath.join(getcwd(), "downloads")
    )
    try:
        makedirs(download_dir, exist_ok=True)
        return disk_usage(download_dir)
    except Exception:
        fallback = ospath.join(getcwd(), "downloads")
        makedirs(fallback, exist_ok=True)
        return disk_usage(fallback)
...
async def get_readable_message(sid, is_user, page_no=1, status="All", page_step=1):
    ...
    usage = _download_disk_usage()
    msg += f"\n┟ <b>CPU</b> → {cpu_percent()}% | <b>F</b> → {get_readable_file_size(usage.free)} [{round(100 - usage.percent, 1)}%]"
