...
async def lifespan(app: FastAPI):
    ...
    try:
        qbittorrent = await create_client(QBITTORRENT_API_URL)
    except Exception as e:
        LOGGER.warning(f"Failed to connect to qBittorrent client: {e}")
    yield
    if aria2:
        await aria2.close()
    if qbittorrent:
...
async def re_verify(paused, resumed, hash_id):
    ...
    while True:
        ...
        res = await client.torrents.files(hash_id)
        verify = True
        for i in res:
        ...
        if paused:
            try:
                await client.torrents.file_prio(
    ...
    if request.method == "POST":
        ...
        try:
            if gid.startswith("SABnzbd_nzo"):
                ...
                client = await get_qbittorrent_client()
                res = await client.torrents.files(gid)
...
async def handle_rename(gid, data):
    try:
        ...
        client = await get_qbittorrent_client()
        if _type == "file":
            await client.torrents.rename_file(hash=gid, **data)
        else:
            await client.torrents.rename_folder(hash=gid, **data)
    except (ClientError, TimeoutError, Exception, AQError) as e:
